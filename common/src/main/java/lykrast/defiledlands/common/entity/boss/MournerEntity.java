...
public class MournerEntity extends Monster implements IEntityDefiled {
    private static final EntityDataAccessor<Integer> INVULNERABILITY_TIME = SynchedEntityData.defineId(MournerEntity.class, EntityDataSerializers.INT);
    protected static final EntityDataAccessor<Byte> MOURNER_FLAGS = SynchedEntityData.defineId(MournerEntity.class, EntityDataSerializers.BYTE);
    ...
    static class MournerMoveControl extends MoveControl {
        ...
        public void tick() {
            if (this.operation == Operation.MOVE_TO) {
                Vec3 vec3 = new Vec3(this.wantedX - mourner.getX(), this.wantedY - mourner.getY(), this.wantedZ - mourner.getZ());
                ...
                if (d0 < mourner.getBoundingBox().getSize()) {
                    ...
                    mourner.setDeltaMovement(mourner.getDeltaMovement().add(vec3.scale(this.speedModifier * 0.05D / d0)));
    ...
    static class MournerChargeGoal extends Goal {
        ...
        @Override
        public boolean canUse() {
            return mourner.getTarget() != null && mourner.getTarget().isAlive() && mourner.distanceToSqr(mourner.getTarget()) <= 16.0D;
        }

        @Override
        public boolean canContinueToUse() {
            return mourner.getMoveControl().hasWanted() && mourner.isCharging() && mourner.getTarget() != null && mourner.getTarget().isAlive() && mourner.distanceToSqr(mourner.getTarget()) <= 25.0D;
        }
        ...
        public void start() {
            ...
            if (target != null) {
                ...
                mourner.getMoveControl().setWantedPosition(vec3.x, vec3.y, vec3.z, 0.5D * (mourner.getRageFactor() + 1));
        ...
        public void tick() {
            ...
            if (mourner.getBoundingBox().inflate(0.5D).intersects(target.getBoundingBox())) {
                ...
                Vec3 vec3 = target.getEyePosition(1.0F);
                mourner.getMoveControl().setWantedPosition(vec3.x, vec3.y - 1.0D, vec3.z, 0.5D * (mourner.getRageFactor() + 1));
            }
        }
    }
    ...
    static class MournerRangedAttackGoal extends Goal {
        ...
        @Override
        public boolean canUse() {
            return mourner.getTarget() != null && !mourner.getMoveControl().hasWanted() && !mourner.isCharging() && mourner.getRandom().nextInt(9 - mourner.getRageFactor() * 2) == 0;
        }
            ...
            if (attackTime <= 0) {
                ...
                switch (mourner.getCurrentAttack()) {
                    case ATTACK_FIREBALLS:
                        ...
                        SmallFireball smallfireball = new SmallFireball(level, mourner, d0 + mourner.getRandom().nextGaussian(), d1, d2 + mourner.getRandom().nextGaussian());
                        ...
                        break;
                    case ATTACK_SHULKER:
                        ShulkerBullet shulkerbullet = new ShulkerBullet(level, mourner, target, mourner.getDirection().getAxis());
                        shulkerbullet.setPos(mourner.getX(), mourner.getY(0.5D) + 0.5D, mourner.getZ());
                        level.addFreshEntity(shulkerbullet);
                        mourner.playSound(SoundEvents.SHULKER_SHOOT, 2.0F, (mourner.getRandom().nextFloat() - mourner.getRandom().nextFloat()) * 0.2F + 1.0F);
                        break;
                    case ATTACK_GHAST:
                        Vec3 vec3 = mourner.getViewVector(1.0F);
                        LargeFireball largefireball = new LargeFireball(level, mourner, target.getX() - (mourner.getX() + vec3.x * 4.0D), target.getY(0.5D) - mourner.getY(1.0D), target.getZ() - (mourner.getZ() + vec3.z * 4.0D), mourner.getRageFactor());
    ...
    static class MournerRandomFlyGoal extends Goal {
        ...
        public void tick() {
            ...
            for (int i = 0; i < 3; ++i) {
                BlockPos blockpos = targetPos.offset(mourner.getRandom().nextInt(15) - 7, mourner.getRandom().nextInt(11), mourner.getRandom().nextInt(15) - 7);
                if (mourner.level().isEmptyBlock(blockpos)) {
                    mourner.getMoveControl().setWantedPosition(blockpos.getX() + 0.5D, blockpos.getY() + 0.5D, blockpos.getZ() + 0.5D, 0.25D);
                    break;
                }
            }
        }
